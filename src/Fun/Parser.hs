...
-- | Parse a constructor
ctorP :: Parser Ctor
ctorP = nilP <|> consP <|> tupP <|> ctorCapP
  where
    nilP = symbol "Nil" >> pure Nil
    consP = symbol "Cons" >> pure Cons
    tupP = symbol "Tup" >> pure Tup

-- | Parse a constructor (capitalized identifier)
ctorCapP :: Parser Ctor
ctorCapP = do
  name <- T.pack <$> ((:) <$> upperChar <*> many alphaNumChar <?> "capitalized constructor")
  pure $ Data name
